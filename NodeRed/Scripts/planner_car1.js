 ...
 *     (flow.get("charger1.allocatedAmp") written by coordinator),
...
// ---- HA inputs ----
const socEnt = global.get("homeassistant.homeAssistant.states['sensor.car1_battery_soc']") || {};   // CHANGE ME: your Car 1 SoC sensor
...
if (!Number.isFinite(soc)) {
    const sinceStr = String(global.get(
        "homeassistant.homeAssistant.states['input_datetime.ev_car1_soc_unavailable_since'].state"
    ));
    ...
    const lastGood = Number(global.get(
        "homeassistant.homeAssistant.states['input_number.ev_car1_soc_last_good'].state"
    ));

    if (Number.isFinite(lastGood) && lastGood >= 0) {
        soc = lastGood;
        soc_source = "last_good";
        node.warn(`Planner car1: SoC sensor unavailable for ${outFor} — planning from last known ${soc}%`);
    } else {
        ...
        soc = Number(global.get(
            "homeassistant.homeAssistant.states['input_number.ev_car1_soc_fallback'].state"
        ));
        soc_source = "fallback";
        if (!Number.isFinite(soc)) {
        ...
        node.warn(`Planner car1: SoC sensor unavailable for ${outFor} and no cached reading — planning from fallback ${soc}%`);
    }
}
...
let soc_stale = false;
{
    ...
    if (Number.isFinite(lu)) {
        ...
        if (Number.isFinite(maxAgeH) && (Date.now() - lu) > maxAgeH * 3600 * 1000) {
            soc_stale = true;
            node.warn(`Planner car1: SoC reading ${soc_age_h} h old — planning from it anyway (sensor updates on change; a parked car keeps its value)`);
        }
    }
}
const battery_kwh = Number(flow.get("car1.battery_kwh") || 75);
const mode = global.get(
    "homeassistant.homeAssistant.states['input_select.ev_car1_mode'].state"
) || "normal";
const target_normal = Number(
    global.get("homeassistant.homeAssistant.states['input_number.ev_car1_target_soc_normal'].state") || 60
);
const target_trip = Number(
    global.get("homeassistant.homeAssistant.states['input_number.ev_car1_target_soc_trip'].state") || 90
);
const target_minimal = Number(
    global.get("homeassistant.homeAssistant.states['input_number.ev_car1_target_soc_minimal'].state") || 40
);
const deadlineStr = global.get(
    "homeassistant.homeAssistant.states['input_text.ev_car1_deadline_time'].state"
) || "07:00";
...
const charger_max_kw = Number(flow.get("car1.max_kw") || 11);
...
const myCharger = chargerOfCar(1);
...
if (allocatedAmp != null && allocatedAmp > 0) {
    ...
    node.warn("Planner car1: no allocatedAmp data, using de-rated charger max");
}
...
if (currentSlot && currentSlot.ts <= deadlineTs) {
    const previousMap = flow.get("car1.allowed_map") || {};
...
// ---- Store and return ----
flow.set("car1.allowed_map", allowedMap);
